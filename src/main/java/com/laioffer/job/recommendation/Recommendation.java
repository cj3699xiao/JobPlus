...
public class Recommendation {
    public List<Item> recommendItems(String userId, double lat, double lon) {
        ...
        // Step 1, get all favored item ids
        ...
        keywordList.sort((Map.Entry<String, Integer> e1, Map.Entry<String, Integer> e2) ->
                Integer.compare(e2.getValue(), e1.getValue())); // .sort with comparotor
        ...
        for (Map.Entry<String, Integer> keyword : keywordList) {
            List<Item> items = client.search(lat, lon, keyword.getKey()); // search according to a keyword
            // if keyword doesn't work, we use title to search
